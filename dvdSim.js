...
(lib.dvdSim = function(mode,startPosition,loop) {
	...
	this.frame_0 = function() {
		/* Animate Horizontally
				...
				*/
				...
				var halfWidth = 107/2;
				var halfHeight = 48/2;
				var randomX = Math.floor((Math.random() * 1280 - halfWidth) + 1 + halfWidth);
				var randomY = Math.floor((Math.random() * 702 - halfHeight) + 1 + halfHeight);
				...
				function fl_AnimateHorizontally()
				{
					...
					if(this.movieClip_1.y + halfHeight >= 720){
                        yval = "Up";
					}
					if(this.movieClip_1.y - halfHeight <= 0){
                        yval = "Down";
					}
					if(this.movieClip_1.x + halfWidth >= 1280){
                        xval = "Left";
					}
					...
					if(currentX != xval || currentY != yval){++hitCounter; console.log(hitCounter);}
	...
	// Layer 1
	this.statBox = new cjs.Text("test", "18px 'Oriya Sangam MN'", "#FFFFFF");
	this.statBox.name = "statBox";
	this.statBox.lineHeight = 20;
	this.statBox.lineWidth = 336;
	this.statBox.setTransform(5,602);
